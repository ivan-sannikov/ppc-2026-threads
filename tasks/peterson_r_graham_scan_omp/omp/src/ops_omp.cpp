...
void ParallelMergeSort(PointSet &points, int left, int right, const Point2D &origin, int depth) {
  if (left >= right) {
    return;
  }
  ...
  if (depth > 4) {
    ...
    std::sort(points.begin() + left, points.begin() + right + 1, comp);
    return;
  }

  // Добавляем скобки для ясности
  int mid = left + ((right - left) / 2);

#pragma omp task shared(points)
  ParallelMergeSort(points, left, mid, origin, depth + 1);

#pragma omp task shared(points)
  ParallelMergeSort(points, mid + 1, right, origin, depth + 1);
...
// Выносим сложную логику в отдельные функции для уменьшения когнитивной сложности
std::size_t FindLowestPointParallel(const PointSet &points) {
...
void SortPointsByAngleParallel(PointSet &points) {
  if (points.size() < 2) {
    return;
  }
...
#pragma omp single
    {
      ParallelMergeSort(points, 1, static_cast<int>(points.size()) - 1, origin, 0);
    }
  }
}
